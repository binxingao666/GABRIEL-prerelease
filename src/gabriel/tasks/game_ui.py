# src/gabriel/tasks/game_ui.py
# ════════════════════════════════════════════════════════════════════
# Game UI analysis task - analyze user interface design elements.
# ════════════════════════════════════════════════════════════════════
...
# ────────────────────────────
# Main UI analysis task
# ────────────────────────────
class GameUI:
    """Analyze game UI design on specified attributes (0–100)."""
    ...
    def __init__(
        ...
        self.template = resolve_template(
            ...
            reference_filename="game_ui_prompt.jinja2",
        )
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        """Return ``df`` with one column per UI attribute rating."""
        ...
        if attr_count > self.cfg.n_attributes_per_run:
            ...
            print(
                f"[GameUI] {attr_count} attributes provided. n_attributes_per_run={self.cfg.n_attributes_per_run}. "
        ...
        for run_idx, df_resp in enumerate(df_resps, start=1):
            ...
            for ident_batch, raw in zip(df_resp.Identifier, df_resp.Response):
                ...
                try:
                    ...
                    if debug:
                        print(f"[GameUI] Skipping malformed identifier {ident_batch}")
                    continue
                if base_ident not in id_to_ratings:
                    if debug:
                        print(f"[GameUI] Skipping unknown identifier {base_ident}")
                    continue
