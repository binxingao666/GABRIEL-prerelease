# src/gabriel/tasks/game_rules.py
# ════════════════════════════════════════════════════════════════════
# Game rules analysis task - extract and analyze game mechanics/rules.
# ════════════════════════════════════════════════════════════════════
...
# ────────────────────────────
# Main rules analysis task
# ────────────────────────────
class GameRules:
    """Analyze game rules and mechanics on specified attributes (0–100)."""
    ...
    def __init__(
        ...
        self.template = resolve_template(
            ...
            reference_filename="game_rules_prompt.jinja2",
        )
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        """Return ``df`` with one column per game rules attribute rating."""
        ...
        if attr_count > self.cfg.n_attributes_per_run:
            ...
            print(
                f"[GameRules] {attr_count} attributes provided. n_attributes_per_run={self.cfg.n_attributes_per_run}. "
        ...
        for run_idx, df_resp in enumerate(df_resps, start=1):
            ...
            for ident_batch, raw in zip(df_resp.Identifier, df_resp.Response):
                ...
                try:
                    ...
                    if debug:
                        print(f"[GameRules] Skipping malformed identifier {ident_batch}")
                    continue
                if base_ident not in id_to_ratings:
                    if debug:
                        print(f"[GameRules] Skipping unknown identifier {base_ident}")
                    continue
