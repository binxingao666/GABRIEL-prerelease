# src/gabriel/tasks/game_storyline.py
# ════════════════════════════════════════════════════════════════════
# Game storyline analysis task - analyze narrative elements in games.
# ════════════════════════════════════════════════════════════════════
...
# ────────────────────────────
@dataclass
class GameStorylineConfig:
    attributes: Dict[str, str]
    save_dir: str = "game_storyline"
...
# ────────────────────────────
# Main storyline analysis task
# ────────────────────────────
class GameStoryline:
    """Analyze game storyline and narrative elements on specified attributes (0–100)."""
    ...
    def __init__(
        ...
        self.template = resolve_template(
            ...
            reference_filename="game_storyline_prompt.jinja2",
        )
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        """Return ``df`` with one column per storyline attribute rating."""
        ...
        if attr_count > self.cfg.n_attributes_per_run:
            ...
            print(
                f"[GameStoryline] {attr_count} attributes provided. n_attributes_per_run={self.cfg.n_attributes_per_run}. "
        ...
        for run_idx, df_resp in enumerate(df_resps, start=1):
            ...
            for ident_batch, raw in zip(df_resp.Identifier, df_resp.Response):
                ...
                try:
                    ...
                    if debug:
                        print(f"[GameStoryline] Skipping malformed identifier {ident_batch}")
                    continue
                if base_ident not in id_to_ratings:
                    if debug:
                        print(f"[GameStoryline] Skipping unknown identifier {base_ident}")
                    continue
